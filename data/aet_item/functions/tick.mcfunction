...
# Grupert's Paw
execute as @e[predicate=aet_item:device/gruperts_paw] if score Tick TickClock matches 1 at @s run summon minecraft:rabbit ~ ~ ~ {RabbitType: 1, CustomName: '[{"text":"Grupert"}]', Health: 100, PersistenceRequired: 1b, Tags: ["Grupert"], ActiveEffects: [{Id: 25, Duration: -1, ShowParticles: 0b}], Attributes: [{Name: "generic.max_health", Base: 100d}]}
execute as @e[type=rabbit,tag=Grupert] if score 15s TickClock matches 1 at @s run function aet_item:device/gruperts_paw
...
# Prisoner Restraints
execute as @a[predicate=aet_item:device/prisoner_restraints] if score Tick TickClock matches 1 at @s if entity @e[type=villager,distance=..1.5] run function aet_item:device/prisoner_restraints
